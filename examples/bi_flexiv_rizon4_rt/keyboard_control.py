...
class KeyboardExit(Exception):  # noqa: N818 — mirrors KeyboardInterrupt: it's a control-flow signal, not an error
...
class KeyboardEpisodeController:
    ...
    * RUNNING: an episode is being recorded. Any of right/left/enter/
    ...
    def start(self) -> None:
        ...
        try:
            from pynput import keyboard  # noqa: PLC0415 — lazy so importing this module stays headless-safe
        except Exception as e:
            raise RuntimeError(
                f"pynput is required for --keyboard_control but could not be imported (headless environment?): {e}"
            ) from e
    ...
    def peek_end_reason(self) -> EpisodeEndReason | None:
        """Return the pending end reason without consuming it."""
        with self._lock:
            return self._end_reason

    def consume_end_reason(self) -> EpisodeEndReason | None:
