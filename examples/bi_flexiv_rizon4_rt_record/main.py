...
Example usage:
    ...
    # Inference with Pico4 intervention (either grip controls its matching arm)
    python -m examples.bi_flexiv_rizon4_rt.main \\
...
import examples.bi_flexiv_rizon4_rt_record.recorder as _recorder
...
class Args:
    ...
    # Recording (LeRobot format, raw 640x480 policy images, optional 400x700
    # tactile images, and absolute actions)
    record: bool = False
    # Record only Pico4 human-intervention frames; each takeover segment is saved
    # as one LeRobot episode. This automatically enables --pico4_intervention.
    ...
    # Match the standard Xense datasets by recording four 700x400 tactile
    # streams in addition to the three 640x480 policy cameras. Tyro exposes
    # --no-record-tactile-images for legacy three-camera datasets.
    record_tactile_images: bool = True
    task: str = (
        "Pick up the optical module with the left hand, and insert it into the network port "
        "with the right hand."
    )

    # Pico4 human-in-the-loop intervention (each grip controls its matching arm)
...
def main(args: Args) -> None:
    if args.record and args.record_intervention:
        raise SystemExit(
            "--record and --record_intervention write different episode semantics. "
            "Run one recording mode at a time."
        )

    if args.record_intervention and not args.pico4_intervention:
        logger.info("--record_intervention enabled: turning on --pico4_intervention")
        args.pico4_intervention = True

    if (args.record or args.record_intervention) and args.record_tactile_images:
        if not args.enable_tactile_sensors:
            logger.info("Tactile recording enabled: turning on --enable-tactile-sensors")
        args.enable_tactile_sensors = True
    ...
    subscribers = []
    if args.record or args.record_intervention:
        if args.dry_run:
            logger.warning(
                "Recording is enabled in dry-run mode — state/action data will not reflect real robot motion"
            )
        recorder = _recorder.make_recorder_subscriber(
            ...
            only_intervention=args.record_intervention,
            include_tactile_images=args.record_tactile_images,
        )
        subscribers.append(recorder)
        if args.record_intervention:
            logger.info(
                f"Intervention recording enabled: repo_id={args.record_repo_id}, task='{args.task}'"
            )
        else:
