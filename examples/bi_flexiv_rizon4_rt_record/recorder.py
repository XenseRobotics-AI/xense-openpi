...
class LeRobotRecorderSubscriber(_subscriber.Subscriber):
    ...
    def __init__(self, dataset: LeRobotDataset, task: str, only_intervention: bool = False):
    ...
    def on_episode_start(self) -> None:
        ...
        if self._only_intervention:
            logger.info("Intervention recording armed")
        else:
    ...
    def on_step(self, observation: dict, action: dict) -> None:
        ...
        if self._only_intervention and not is_intervention:
            if self._step_count > 0:
                self._save_current_episode("intervention segment ended")
            return
    ...
    def on_episode_end(self) -> None:
        if self._step_count == 0:
            if self._only_intervention:
                logger.info("Episode ended with no intervention steps — not saving")
            else:
                logger.warning("Episode ended with 0 steps — not saving")
            return
        self._save_current_episode("episode ended")

    def _save_current_episode(self, reason: str) -> None:
        logger.info(f"Saving episode ({self._step_count} steps, {reason})...")
...
def make_recorder_subscriber(
    ...
    Args:
        ...
        only_intervention: Only record frames whose action dict carries
            is_intervention=True. Each contiguous intervention segment is saved
            as a separate LeRobot episode.
