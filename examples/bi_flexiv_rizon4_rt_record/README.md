...
| `--record_intervention` | `False` | Record only Pico4 takeover frames as LeRobot episodes; enables `--pico4_intervention` |
...
1. `--dry_run --pico4_intervention` — hold either grip and confirm only its matching arm moves
...
(and `False` otherwise). Use `--record_intervention` to save only those takeover
frames into a LeRobot dataset. Each contiguous takeover segment is saved as one
LeRobot episode, and normal policy-controlled frames are skipped.
...
Record only Pico4 human-intervention demonstrations:

```bash
mamba run -n lerobot-xense python -m examples.bi_flexiv_rizon4_rt.main \
    --host 10.142.1.1 --port 8000 \
    --record_intervention \
    --record_repo_id Xense/my_intervention_dataset \
    --task "pack 6 cosmetic bottles into the carton"
```

`--record_intervention` automatically enables Pico4 intervention and saves one
LeRobot episode per continuous takeover segment. It is mutually exclusive with
`--record`.
