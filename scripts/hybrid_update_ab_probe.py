...
def main():
    ...
    graphdefs = {}
    for name, start, count in hybrid_ranges:
        model_config = dataclasses.replace(
            cfg_on.model,
            use_cudnn_attention=True,
            cudnn_attention_layer_start=start,
            cudnn_attention_num_layers=count,
        )
        graphdefs[name] = nnx.graphdef(model_config.create(init_rng))

    fn = jax.jit(functools.partial(_loss_and_update, gdef_off, cfg.trainable_filter, state.tx, 0.0))
    with sharding.set_mesh(mesh):
        loss, updates = fn(state.opt_state, state.params, step_rng, obs, act)
        reference = _to_host(updates)
    del updates, fn
    print(f"computed explicit reference: loss={float(loss):.8f}", flush=True)

    for name, _, _ in hybrid_ranges:
        fn = jax.jit(functools.partial(_loss_and_update, graphdefs[name], cfg.trainable_filter, state.tx, 0.0))
        with sharding.set_mesh(mesh):
            loss, updates = fn(state.opt_state, state.params, step_rng, obs, act)
            candidate = _to_host(updates)
        del updates, fn
        print(f"computed {name}: loss={float(loss):.8f}", flush=True)
        _print_comparison(name, reference, candidate)
        del candidate
