...
日期：2026-09-18（第 6 版）。状态：步骤 2 主实验（0917 配置，m=10、λ=1）的 10k checkpoint
（`checkpoints/pi05_base_bi_flexiv_bottle_sorting_0917_fastvit_ltp_h100/10000`）已用步骤 1 的两个探针
加一个"头的反事实"验收，结论见 §6.1 末"步骤 2 首跑"：LTP 目标被视觉 / state 捷径满足，头对触觉
shuffle 零响应，expert 仍不消费触觉；下一轮的变量见该节末尾。步骤 1 的结论（触觉 token 有信息、
expert 不消费，选层 `l10`）不变。方案本身不依赖任何已有 checkpoint 的结果。

第 6 版改动：写入步骤 2 首跑（0917 LTP 10k）的三项测量与 wandb 曲线读数，新增
`scripts/probe_ltp_head_counterfactual.py`（头的反事实）与 `scripts/refit_layer_probe_latent.py`
（保存特征对 latent 目标离线重拟合）；§3 步骤 2 加"首跑结论与下一轮变量"。
...
- 首跑结论（2026-09-18，详见 §6.1 末"步骤 2 首跑"）：主配置未过线。`L_tac` 到 0.10 但对触觉 shuffle
  零响应，`S_vl` 在 l9–l10 涨 10 倍而 `S_tac` 仍 1e-5，`r_g` 2–4 超标、flow loss 比 1a 差 26%。LTP 目标
  被 prefix（RGB + state）捷径满足。下一轮变量按 §6.1 末的顺序：RGB 遮蔽堵捷径 → λ 降 → delta 目标。
  上面四个对照（接口 / 读出 / 目标 / 多层）里，"目标对照"提前；接口与读出对照在捷径堵住前没有意义。
...
**步骤 2 首跑（2026-09-18，`pi05_base_bi_flexiv_bottle_sorting_0917_fastvit_ltp_h100/10000`：m=10、
K/V 只读动作位置、latent 目标、λ=1、头 4× LR，8×H100 batch 256；本机 5090 单卡验收，与步骤 1 同 seed
同帧。输出 `outputs/tactile_layer_probes/linear/ltp10k_2048/`、`sensitivity/ltp10k_512/`、
`head_shuffle/ltp10k/`、`linear/latent_refit/`）。**

wandb `svi09dbw` 读数（训练集上的 `L_tac`，零预测基线 = 1）：

| step | `loss/flow` | 0915 同步 | `loss/tac` | τ 箱 0 / 3 | `aux/grad_ratio` |
|---|---|---|---|---|---|
| 1000（warm-up 结束） | 1.59e-2 | 1.52e-2 | 0.967 | 0.970 / 0.967 | 3.0 |
| 3000 | 9.6e-3 | 7.3e-3 | 0.252 | 0.260 / 0.250 | 2.8 |
| 10000 | 4.29e-3 | 3.40e-3 | 0.106 | 0.107 / 0.105 | 2.4 |

- `L_tac` 在 warm-up 结束后 2k 步内从 1 掉到 0.25，10k 时 0.106，比"把当前 latent 原样外推"的持久性
  基线（同帧 nMSE 0.487）低得多。
- 四个 τ 箱始终持平：动作条件没被用上（§2.3.4 的监控项）。
- `r_g` 全程 1.7–4.2，远高于目标区间 0.1–0.5：Block 1..10 的梯度由 `L_tac` 主导。代价是 `loss/flow`
  比 0915 同步数高 26%（4.29e-3 vs 3.40e-3），`grad_norm` 高 8×。

**头的反事实（决定性）。** `scripts/probe_ltp_head_counterfactual.py`：头开着，同 512 帧、同 rng
（τ 与噪声配对）× 4 个 seed，换触觉看 `L_tac`：

| 条件 | `L_tac` | 接触 | 非接触 | `L_flow` |
|---|---|---|---|---|
| real | 0.0971 | 0.1186 | 0.0756 | 4.13e-3 |
| tac-shuffle | 0.0971 | 0.1186 | 0.0756 | 4.13e-3 |
| tac-zero | 0.0972 | 0.1187 | 0.0757 | 4.17e-3 |
| null | 0.1027 | 0.1247 | 0.0806 | 5.31e-3 |

头的预测对触觉内容零响应（shuffle 与 real 到第四位相同；null 高出的 0.006 是去掉 4 个 key 改变了
attention 归一化，与 §6.1 步骤 1 的 flow loss 观察同源）。头把 90% 的方差解释掉，一点触觉都没用。

**敏感度探针（τ=1.0，其余 τ 同形）。** 与步骤 1a 同帧对比：

| 测量点 | S_tac 0915 → 0917 | S_vl 0915 → 0917 | share 0915 → 0917 |
|---|---|---|---|
| l9 | 2.6e-6 → 1.8e-5 | 0.025 → 0.30 | 2.5e-6 → 1.7e-5 |
| l10 | 3.5e-6 → 1.9e-5 | 0.045 → 0.81 | 3.3e-6 → 1.8e-5 |
| l18 | 3.1e-5 → 6.6e-5 | 0.63 → 0.99 | 2.8e-5 → 5.9e-5 |
| v_t | 1.2e-6 → 1.6e-6 | 0.091 → 0.091 | 1.1e-6 → 1.4e-6 |

`S_tac` 在 l10 涨了 5 倍但仍在 1e-5 量级，`v_t` 与最终 chunk 不动；`S_vl` 在 l9–l10 从 0.03–0.05 跳到
0.30–0.81。LTP 把 VL prefix（视觉 + 离散 state）的信息大量拉进了 l9–l10 的动作位置，不是触觉。
自检：real 重复前向逐位相等，四个触觉变体像素均变了。

**线性探针。** 像素场目标：谷底从 `l10` 移到 `l6`（0.964，不变），`l10` 本身升到 0.989–0.994（被 LTP
重组，mean-pool 线性读不出像素场）；`null − real` 增益在 l8–l9 达 2e-3，比步骤 1 的 ±8e-4 大三倍，
但仍是千分位。latent 目标（LTP 实际训练的目标，`scripts/refit_layer_probe_latent.py` 用保存的特征
离线重拟合，按 episode 切验证集）：0915 全层 0.993（读不出），0917 在 `l10` 陡降到 0.854，之后沿层
回升到 `l18` 的 0.934；但 `null − real` 增益最大只有 +0.003。可读出的 15% 几乎全来自 prefix。

**结论。** 步骤 2 的过线条件（"换触觉引起的动作系统性变化"）未达成，属 §3 步骤 2 里"头预测得好但动作
不变"的情形，而且更进一步：头本身就没用触觉。机理：单任务录制下，未来触觉 latent（慢变的凝胶外观 +
接触状态）从 RGB + 夹爪 state 就能预测；动作位置每层都能 attend 到 prefix，最短路径是 prefix 而不是
触觉 token。§2.3.3 只堵了"头直接读触觉位置"这一条捷径，没堵 prefix 这条。RATG 的 taxel 阵列信号与
视觉的冗余小，所以没暴露这个问题。10k 之后继续训练不会改变这一点：`L_tac` 已被 prefix 路径解释到
0.1，剩余梯度没有理由流向触觉。

**30k checkpoint 复验（2026-09-18，同一 run 的 `30000`，同 seed 同帧，本机 5090。输出
`outputs/tactile_layer_probes/{head_shuffle/ltp30k, sensitivity/ltp30k_512, linear/ltp30k_2048}`，latent 重拟合
并入 `linear/latent_refit/`）。** 问题：10k 时"头不用触觉"是不是训练不够。答案是否：继续训练 20k 步把
`L_tac` 和 flow loss 都压低了，但全部走 prefix 路径，触觉依赖度反而退回步骤 1a 的水平。

头的反事实（512 帧 × 4 seed，配对 τ 与噪声）：

| 条件 | `L_tac` 10k → 30k | 接触 30k | 非接触 30k | τ 四箱 30k | `L_flow` 10k → 30k |
|---|---|---|---|---|---|
| real | 0.0971 → 0.0569 | 0.0702 | 0.0436 | 0.055 / 0.059 / 0.054 / 0.058 | 4.13e-3 → 1.93e-3 |
| tac-shuffle | 0.0971 → 0.0569 | 0.0702 | 0.0436 | 同上 | 1.93e-3 |
| tac-zero | 0.0972 → 0.0569 | 0.0702 | 0.0436 | 同上 | 1.93e-3 |
| null | 0.1027 → 0.0612 | 0.0758 | 0.0466 | 0.059 / 0.063 / 0.058 / 0.062 | 5.31e-3 → 6.15e-3 |

- `L_tac` 降了 41%，但 real / tac-shuffle / tac-zero 到第四位仍完全相同：头对触觉内容的响应还是零。
  τ 四箱持平，动作条件仍未被用上。null 的差距（0.004）与 10k 一样只是去掉 4 个 key 的归一化效应。
- 这些帧全在训练 episode 里，0.057 含记忆成分；但要点不在绝对值，在"换触觉不动"。

敏感度探针（τ=1.0，同 512 帧；括号内 τ=0.25）：

| 测量点 | S_tac 0915 / 10k / 30k | S_vl 0915 / 10k / 30k | share 0915 / 10k / 30k |
|---|---|---|---|
| l9 | 2.6e-6 / 1.8e-5 / 9.2e-6 | 0.025 / 0.30 / 0.25 | 2.5e-6 / 1.7e-5 / 8.6e-6 |
| l10 | 3.5e-6 / 1.9e-5 / 1.3e-5 | 0.045 / 0.81 / 0.78 | 3.3e-6 / 1.8e-5 / 1.3e-5 |
| l18 | 3.1e-5 / 6.6e-5 / 5.4e-5 | 0.63 / 0.99 / 0.99 | 2.8e-5 / 5.9e-5 / 4.9e-5 |
| v_t | 1.2e-6 / 1.6e-6 / 1.3e-6 | 0.091 / 0.091 / 0.091 | 1.1e-6 / 1.4e-6 / 1.2e-6 (2.8e-6 / 3.4e-6 / 2.8e-6) |
| chunk（10 步） | 2.2e-6 / 2.1e-6 / 2.1e-6 | 1.13 / 1.13 / 1.13 | 1.9e-6 / 1.8e-6 / 1.9e-6 |

- 10k 时 `S_tac` 在 l9–l10 涨的那 5 倍到 30k 回吐了一半，`v_t` 与最终 chunk 的 share 退回 0915 的
  1.1e-6 / 1.9e-6；接触 / 非接触子集在四个 τ 下仍无差别（1.16e-6 vs 1.19e-6）。`S_vl` 在 l9–l10 维持
  0.25–0.78，VL 捷径没有松动。自检通过（real 重复前向逐位相等、四个触觉变体像素均变；触觉 token
  `S_x` 1.075 未塌缩；`v_t` 处 `S_pad/S_vl` 0.013–0.028）。
- 触觉支路本身在动：token 对 `tac-timeshift`（30 帧）的敏感度从 0.43 升到 0.80，即 30k 的 token 更能区分
  相隔 1 秒的触觉图。参数差分（10k→30k 的 `‖Δθ‖/‖θ‖`）：`tactile_proj/bias` 1.52、`tactile_proj/kernel`
  0.05、FastViT 主体 <0.007，头 0.28，expert 0.07，VLM 0.02。变化集中在投影层的偏置，是 token 幅度 /
  偏移的调整，不是内容重编码；无论怎么变，下游没人读。

线性探针（2048 帧，按 episode 切验证集）：

- 像素场目标：谷底仍在 `l6`，0.9654 → 0.9590（比 0915 的 `l10` 谷底 0.9637 略好）；`l10` 0.989 → 0.987。
  10k 时 l8–l9 的 `null − real` 增益 2e-3 到 30k 缩回 ±7e-4，与 0915 同级；`tac-shuffle − real` 仍在 1e-6。
- latent 目标：`l10` 0.854 → 0.850，`null − real` 增益 +0.003 → +0.004，沿层形状不变（l9 0.971、l18 0.942）。
  可读出的 15% 仍几乎全来自 prefix，20k 步只多挤出千分之一。
- 副产品：同一批训练 episode 上 flow loss（τ=0.25）real 7.3e-3 → 3.05e-3（0915 10k 为 5.46e-3），
  但 null 从 9.4e-3 升到 11.3e-3，即 null/real 从 1.3× 变成 3.7×。模型对"触觉 token 在不在"的依赖在加深
  （归一化捷径），对"触觉 token 是什么"仍是零（shuffle 与 real 到第五位相同）。

**结论不变，且更硬：** 训练时长不是变量。`L_tac` 从 0.10 到 0.057 的全部进步来自 prefix 路径，触觉
敏感度沿层整体回落，头对 shuffle 零响应。下一轮必须改结构性变量（堵 RGB 捷径 / 降 λ / 换 delta 目标），
不要再拿同配置的更晚 checkpoint 复验。

下一轮（一次一个变量，每个都先过"头的反事实"再谈动作）：

1. **堵 VL 捷径（首选）**：§3 可选 1c 的 `rgb_mask_prob` 高比例遮三路 RGB 与 LTP 同开（N0 Stage 2）。
   RGB 被遮时 prefix 只剩 prompt + state，未来触觉的唯一来源变成触觉 token。零模型改动。state 仍在
   prompt 里，若 `L_tac` 在遮 RGB 后仍对 shuffle 不响应，说明夹爪 state 也够用，再考虑连 state 一起遮。
2. **λ 降到 0.1–0.25**：把 `r_g` 拉回 0.1–0.5，收回 26% 的 flow loss 代价。与第 1 项分开跑。
3. **目标换 delta**（`pixel_delta` 或 `z_{t+k} − z_t`）：去掉"当前状态"这部分方差，只留下接触变化。
   持久性基线对它无效，视觉对它的解释力也小（像素场线性探针 0.964）。是 §3 的"目标对照"，一次一个。
4. 工程：`L_tac` 留出 episode（现在 160 个 episode 全在训练集里，0.097 可能含记忆）；训练里加
   `L_tac[tac-shuffle]` 的周期性监控，把头的反事实变成在线指标。
